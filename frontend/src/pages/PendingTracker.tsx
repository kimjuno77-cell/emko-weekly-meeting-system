...
// 설명: 고성능 리치 텍스트 에디터 및 서식 뷰어 컴포넌트 임포트
...
const PendingTracker = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
                  ...
                  >
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="space-y-1">
                        ...
                        {/* 설명: 리치 텍스트 서식 및 캡처 이미지를 선명하게 표시하고 클릭 확대(라이트박스)를 지원하는 뷰어 적용 */}
                        {item.description && (
                          <div className="max-w-2xl mt-1">
                            <RichTextViewer content={item.description} compact={false} className="text-xs text-slate-600" />
                          </div>
                        )}
      ...
      {/* CRUD 모달 창 - 리치 텍스트 편집 및 캡처 이미지를 쾌적하게 작성할 수 있도록 너비 확장 및 스크롤 구조 적용 */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[92vh] flex flex-col shadow-2xl border border-slate-100 overflow-hidden animate-zoomIn">
            ...
            <form onSubmit={handleSaveItem} className="p-6 space-y-4 overflow-y-auto">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5">설명 및 세부 계획 (선택, 화면 캡처 Ctrl+V 붙여넣기 가능)</label>
                <RichTextEditor
                  value={itemDesc}
                  onChange={(content) => setItemDesc(content)}
                  placeholder="세부적인 이슈 설명과 해결 계획을 작성해 주세요. (화면 캡처 복사 후 Ctrl+V로 붙여넣기 가능)"
                  minHeight="140px"
                />
              </div>
