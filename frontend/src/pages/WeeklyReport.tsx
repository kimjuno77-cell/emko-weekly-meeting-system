...
const WeeklyReport = () => {
  ...
  const fetchReportData = async () => {
    try {
      ...
      // 3. 해당 주차의 활성화된 고우선순위 Pending 항목 조회
      ...
      if (!pendingErr && pendingData) {
        setHighPriorityPending(pendingData);
      }
    } catch (error) {
  ...
  // 요약 통계 계산
  const progressCount = weeklyUpdates.reduce(
    (sum, u) => sum + (u.tasks?.filter((t) => t.task_type === 'progress').length || 0),
    0
  );
  const issueCount = weeklyUpdates.reduce(
    (sum, u) => sum + (u.tasks?.filter((t) => t.task_type === 'issue').length || 0),
    0
  );
  const planCount = weeklyUpdates.reduce(
    (sum, u) => sum + (u.tasks?.filter((t) => t.task_type === 'plan').length || 0),
    0
  );

  const submittedCount = weeklyUpdates.filter(
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm print:hidden">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="text-xs font-bold text-sky-600 hover:bg-sky-50 px-2.5 py-1.5 rounded-lg transition"
          >
            이번 주
          </button>
        </div>
        ...
        >
          <Printer className="h-4 w-4" /> 리포트 인쇄 (A4 최적화)
        </button>
      </div>
      ...
      {loading ? (
        ...
        /* 실제 인쇄 및 화면에 표시될 보고서 레이아웃 */
        <div className="bg-white rounded-3xl border border-slate-100 shadow-lg p-6 sm:p-10 space-y-10 print:border-0 print:shadow-none print:p-0">
          ...
          <div className="space-y-4">
            <h2 className="text-lg font-extrabold text-slate-900 flex items-center gap-2 border-l-4 border-slate-900 pl-2">
              Ⅰ. 종합 현황 요약
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50 rounded-2xl p-5 border border-slate-100">
              <div className="text-center space-y-1">
                ...
                <p className="text-2xl font-black text-slate-800">
                  {submittedCount} <span className="text-xs font-semibold text-slate-500">/ {teams.length}개 팀</span>
                </p>
              </div>
          ...
          {/* 2. 핵심 이슈 및 장애 요약 (Critical Issues) */}
          <div className="space-y-4">
            <h2 className="text-lg font-extrabold text-slate-900 flex items-center gap-2 border-l-4 border-rose-500 pl-2">
              Ⅱ. 주요 이슈 및 장애 리스크
            </h2>
            {criticalIssues.length === 0 ? (
              <div className="p-4 bg-slate-50/50 border border-slate-100 rounded-xl text-center text-xs text-slate-500">
                이번 주차에 보고된 특별한 지연 사항이나 장애 위험 요인이 없습니다.
              </div>
            ) : (
              <div className="border border-rose-100 rounded-2xl overflow-hidden print:overflow-visible print:border-0 shadow-sm print:shadow-none">
                    ...
                    {criticalIssues.map((issue) => (
                      <tr key={issue.id} className="hover:bg-rose-50/10">
                        ...
                        <td className="px-4 py-3 space-y-1">
                          ...
                          {issue.description && (
                            <p className="text-slate-500 whitespace-pre-wrap">{issue.description}</p>
                          )}
                        </td>
          ...
          {/* 3. 중요 Pending 항목 (High Priority Pending) */}
          <div className="space-y-4">
            <h2 className="text-lg font-extrabold text-slate-900 flex items-center gap-2 border-l-4 border-amber-500 pl-2">
              Ⅲ. 중점 관리 Pending 항목
            </h2>
            {highPriorityPending.length === 0 ? (
              <div className="p-4 bg-slate-50/50 border border-slate-100 rounded-xl text-center text-xs text-slate-500">
                추적 중인 중요 Pending 과제가 없습니다.
              </div>
            ) : (
              <div className="border border-slate-100 rounded-2xl overflow-hidden print:overflow-visible print:border-0 shadow-sm print:shadow-none">
            ...
            <div className="space-y-8">
              {teams.map((team) => {
                ...
                return (
                  <div
                    key={team.id}
                    className="border border-slate-200 rounded-2xl p-6 space-y-4 shadow-sm hover:border-slate-300 transition break-inside-avoid"
                  >
                    {/* 팀 보고서 타이틀 */}
                    ...
                    {!update ? (
                      <p className="text-xs text-slate-400 py-4 text-center">
                        이번 주차에 등록된 업무 상세 사항이 없습니다.
                      </p>
                    ) : (
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {/* 주요 진행 사항 */}
                        <div className="space-y-2">
                          <h4 className="text-xs font-bold text-sky-700 bg-sky-50 px-2 py-1 rounded">
                            ✓ 주요 진행 사항
                          </h4>
                          {progressList.length === 0 ? (
                            <p className="text-[11px] text-slate-400 pl-2">진행 업무가 없습니다.</p>
                          ) : (
                            <ul className="space-y-2 pl-2">
                              {progressList.map((t) => (
                                <li key={t.id} className="text-xs text-slate-700 list-disc list-inside space-y-0.5">
                                  ...
                                  {t.description && (
                                    <p className="text-[10px] text-slate-500 pl-4 leading-normal">
                                      {t.description}
                                    </p>
                                  )}
                                </li>
                        ...
                        {/* 특이사항 및 이슈 */}
                        <div className="space-y-2">
                          <h4 className="text-xs font-bold text-rose-700 bg-rose-50 px-2 py-1 rounded">
                            ✓ 주요 이슈 및 장해요인
                          </h4>
                          {issueList.length === 0 ? (
                            <p className="text-[11px] text-slate-400 pl-2">특이사항이 없습니다.</p>
                          ) : (
                            <ul className="space-y-2 pl-2">
                              {issueList.map((t) => (
                        ...
                        {/* 다음주 계획 */}
                        <div className="space-y-2">
                          <h4 className="text-xs font-bold text-emerald-700 bg-emerald-50 px-2 py-1 rounded">
                            ✓ 차주 업무 계획
                          </h4>
                          {planList.length === 0 ? (
                            <p className="text-[11px] text-slate-400 pl-2">다음 주 계획이 없습니다.</p>
                          ) : (
                            <ul className="space-y-2 pl-2">
                              {planList.map((t) => (
                                <li key={t.id} className="text-xs text-slate-700 list-disc list-inside space-y-0.5">
